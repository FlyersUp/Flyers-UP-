...
export function QuickRulesSheet({ open, onContinue, onClose, bookingReview }: QuickRulesSheetProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-black/15 dark:bg-black/50"
    ...
    >
      <div
        ref={panelRef}
        className="w-full max-w-lg bg-[#F5F5F5] dark:bg-[#171A20] rounded-t-2xl shadow-xl p-6 pb-[env(safe-area-inset-bottom)]"
        style={{ maxHeight: '85vh' }}
        onClick={(e) => e.stopPropagation()}
      >
        <h2 id="quick-rules-title" className="text-lg font-semibold text-[#111111] dark:text-[#F5F7FA] mb-6">
          Quick Rules
        </h2>
        ...
        <div className="flex flex-col gap-3">
          ...
          <Link
            href="/booking-rules"
            onClick={onClose}
            className="w-full h-12 rounded-full text-sm font-medium text-[#6A6A6A] dark:text-[#A1A8B3] border border-black/15 dark:border-white/10 hover:bg-black/5 dark:hover:bg-white/5 transition-colors flex items-center justify-center"
          >
            View full rules
          </Link>
        </div>

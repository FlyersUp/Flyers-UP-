  ...
  return (
    <MarketingSection id="how-it-works" className="bg-market-linen">
      <div className="text-center">
        <p className="text-sm font-semibold uppercase tracking-wide text-market-sage">How it works</p>
        <h2 className="mt-2 text-3xl font-bold tracking-tight text-market-slate sm:text-4xl">
          Three calm steps
        </h2>
        <p className="mx-auto mt-3 max-w-2xl text-lg text-market-charcoal/75">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-3 md:gap-8">
        {steps.map(({ n, title, description, Icon }) => (
          <div
            key={title}
            className="rounded-2xl border border-market-line bg-market-cloud/90 p-6 text-center shadow-[0_2px_12px_rgba(45,52,54,0.06)] md:text-left"
          >
            ...
            <div className="mx-auto mb-4 flex h-fit w-fit items-center justify-center rounded-full bg-market-cloud p-4 text-market-slate ring-1 ring-market-line/80 md:mx-0">
              <Icon className="h-8 w-8 sm:h-9 sm:w-9" />
            </div>
            <h3 className="text-lg font-semibold text-market-slate">{title}</h3>
            <p className="mt-2 text-sm leading-relaxed text-market-charcoal/80">{description}</p>
          </div>
        ))}
      </div>
    </MarketingSection>

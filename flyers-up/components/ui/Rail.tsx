...
export function Rail({ className = '', showLabel = false, label }: RailProps) {
  ...
  return (
    <div className={`relative w-[4px] bg-[#F5F5F5] dark:bg-[#18181B] border-r border-[#E5E5E5] dark:border-white/10 shrink-0 ${className}`}>
      ...
      {showLabel && (
        ...
        >
          <span className="text-[#6B7280] dark:text-[#A1A1AA]">{displayLabel}</span>
        </div>
      )}
    </div>
  );
